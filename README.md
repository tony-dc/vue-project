...
​     具体实现步骤：引入discount ，movieSeat组件并注册挂载，获取父组件传过来的vip和list数据。然后在计算属性中处理一些需要拿到的数据。在data中初始化一个当前索引值为0，引入swipper第三方插件处理图片滑动效果。根据监听到的索引值动态显示对应的数据列表中的数据。将计算属性中处理后的tips，和info传递给子组件。
